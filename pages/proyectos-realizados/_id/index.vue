  ...
  <div class="general pb-24">
    ...
    <div class="flex flex-col">
      ...
      <div class="w-full flex flex-col justify-center px-6 sm:px-10">
        <img class="w-full object-fill h-full" :src="'http://13.38.83.185:1337' + feature" alt="Paseo Mallorca 15" />
        <ul class="mt-5 grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-4 gap-4 px-0">
          <li @click="changeImg(item.attributes.formats.large.url)" class="cursor-pointer sm:opacity-75 hover:opacity-100" v-for="(item, i) in images" :key="i">
            <img class="w-full object-fill h-full" :src="'http://13.38.83.185:1337' + item.attributes.formats.small.url" />
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  mounted () {
    axios
      .get('http://13.38.83.185:1337/api/posts/' + this.$route.params.id + '?populate=*')
      .then(response => (
