<template>
    <div class="general pb-24">
      <Header
        :color-white="false"
        :headline="projectos().projecto.fields.title"
        :go-back-link="goBackLink"
      />
      <div v-if="!projectos().loading" class="flex flex-col">
        <div class="w-full flex flex-col justify-center px-6 sm:px-10">
          <img class="w-full object-fill h-full" :src="activeImage" alt="Paseo Mallorca 15" />
          <ul class="mt-5 grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-4 gap-4 px-0">
            <li @click="handleDynamicImage(item.fields.file.url)" class="cursor-pointer sm:opacity-75 hover:opacity-100" v-for="(item, i) in projectos().projecto.fields.images" :key="i">
              <img class="w-full object-fill h-full" :src="item.fields.file.url" />
            </li>
          </ul>
        </div>
      </div>
  ...
  <script setup>
    import { projectos } from '~/store/proyectos'
